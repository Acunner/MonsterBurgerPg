...
if($method==='GET' && $path==='/products'){
    auth_staff();
    $all=[];
    foreach(['LANCHES','COMBOS','ACOMPANHAMENTOS','BEBIDAS','INGREDIENTES','EXTRAS'] as $t){
...
if($method==='PUT' && preg_match('#^/products/([A-Z_]+)/(\d+)$#',$path,$pm)){
    auth_staff();
    $allowed=['LANCHES','COMBOS','ACOMPANHAMENTOS','BEBIDAS','INGREDIENTES','EXTRAS'];
    $table=$pm[1]; $id=(int)$pm[2];
    if(!in_array($table,$allowed)) json_err('Tabela inválida.');
    $b=body();
    $fields=[];$vals=[];
    $editable=['nome','descricao','ingredientes','ml','preco','promocao','valor_promocional','marca','retornavel','img','rate','preco_de_compra','estoque'];
...
if($method==='POST' && preg_match('#^/products/([A-Z_]+)$#',$path,$pm)){
    ...
    $b=body();
    db()->prepare(
        "INSERT INTO `$table` (nome,descricao,ingredientes,ml,preco,promocao,valor_promocional,marca,retornavel,img,rate,preco_de_compra,estoque)
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)"
    )->execute([
        $b['nome']??'', $b['descricao']??'', $b['ingredientes']??'', (int)($b['ml']??0),
